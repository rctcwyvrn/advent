...
for passport in passports:
    ...
    if "byr" in fields and "iyr" in fields and "eyr" in fields and "hgt" in fields and "hcl" in fields and "ecl" in fields and "pid" in fields:
        byr = int(values[fields.index("byr")])
        c1 = byr >= 1920 and byr <= 2002

        iyr = int(values[fields.index("iyr")])
        c2 = iyr >= 2010 and iyr <= 2020

        eyr = int(values[fields.index("eyr")])
        c3 = eyr >= 2020 and eyr <= 2030

        hgt = values[fields.index("hgt")]
        unit = hgt[-2:]
        h = int(hgt[:-2])
        c4 = (unit == "in" and h >= 59 and h <=76) or (unit == "cm" and h >= 150 and h <=193)

        hcl = values[fields.index("hcl")]
        c5 = hcl[0] == "#" and re.match("[0-9a-f]*", hcl[1:]) and len(hcl) == 7

        ecl_valid = ["amb", "blu", "brn", "grn", "gry", "hzl", "oth"]
        c6 = values[fields.index("ecl")] in ecl_valid

        pid = values[fields.index("pid")]
        c7 = len(pid) == 9 and re.match("[0-9]*", pid)
